...
@register_model("pytorch")
def resnet50_sophie_base61(model_name, *args):
    import torchvision.models as zoomodels
    model = zoomodels.__dict__["resnet50"](pretrained=False)
    #TODO: Update the paths from local to hosted
    checkpoint = torch.load("/home/local/data/sophie/imagenet/output/FullGPU/base/256/model_61.pth")
    model.load_state_dict(checkpoint["model"])
    return PytorchModel(model, model_name, *args)
...
@register_model("pytorch")
def resnet50_sophie_grey61(model_name, *args):
    import torchvision.models as zoomodels
    model = zoomodels.__dict__["resnet50"](pretrained=False)
    checkpoint = torch.load("/home/local/data/sophie/imagenet/output/FullGPU/grey/256/model_61.pth")
    model.load_state_dict(checkpoint["model"])
    return PytorchModel(model, model_name, *args)
