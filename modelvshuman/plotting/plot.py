...
def get_mean_over_datasets(colname,
    ...
    for d_orig in datasets:
        ...
        for dmaker in decision_maker_fun(df_raw):
            ...
            result_df.loc[len(result_df)] = [pd.NA, dmaker.plotting_name,d.name, r1[metric_name], dmaker.color]
...
def plot_shape_bias_boxplot(datasets,
    ...
    for dmaker in decision_maker_fun(df):
        ...
        # print(f"else dc: {dmaker.decision_makers[0]} \ndmaker_human: {dmaker_human}")
        # else dc: subject-01 dmaker_human: subject-10
        # if subject_name in consts.TORCHVISION_MODELS:
        if dmaker.decision_makers[0] in consts.TORCHVISION_MODELS:
                    ...
                    for condition in conditions:
                        for j, g2 in enumerate(group_2):
                            ...
                            row = _analyse_inner(df=df, model=g1, human=g2,
                                                 ...
                                                 colname=colname,
                                                 condition=condition)
                            #row: {'model': 'resnet50_sophie_base89', 'human': 'subject-01', 'dataset': 'edge', 'metric': 'accuracy difference', 'condition': None, 'value': 0.4389062500000001}
                            rows_list.append(row)
        # ['model' 'human' 'dataset' 'metric' 'condition' 'value']
        
        # print(result_df.columns.values)
        # result_df.loc[len(result_df)] =[rows_list]
        result_df = pd.concat([result_df, pd.DataFrame(rows_list)]).reset_index(drop=True)
...
def format_benchmark_df(df, decision_makers, metric_names,
    ...
    if include_humans:
        ...
        # print(human_dict) : {'model': 'humans', 'accuracy difference': 0.020350694444444442, 'observed consistency': 0.8386111111111111, 'error consistency': 0.3184364338405932}
        df.loc[len(df)] = human_dict
